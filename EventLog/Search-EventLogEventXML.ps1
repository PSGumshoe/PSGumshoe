function Search-EventLogEventXML {
   ...
   process {
       ...
       switch ($PSCmdlet.ParameterSetName) {
           ...
           Default {
               if ($MaxEvents -gt 0) {
                   Get-WinEvent -FilterXml $BaseFilter -MaxEvents $MaxEvents | ConvertFrom-EventEventXMLRecord
               } else {
                   Get-WinEvent -FilterXml $BaseFilter | ConvertFrom-EventEventXMLRecord
               }
           }
       }
   }
